...
import { AlertTriangle, CheckCircle2, Lightbulb, Mail, ExternalLink } from "lucide-react";
...
const AdminDashboardPage = () => {
    ...
    (async () => {
      ...
      if (ordersData) {
        setRecentOrders(ordersData.map((o: any) => ({
          ref: o.ref, client: o.client_name,
  ...
  const kpis = [
    { label: "Commandes", value: String(orderCount), color: "text-primary" },
    { label: "Chiffre d'affaires", value: `${totalCA.toLocaleString("fr-FR")} €`, color: "text-primary" },
    { label: "Leads", value: String(leadCount), color: "text-primary" },
    { label: "Leads non traités", value: String(unprocessedLeads), color: unprocessedLeads > 0 ? "text-destructive" : "text-primary" },
  ];
  ...
  return (
    <div className="space-y-6 font-sans">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Bienvenue, {admin?.name || "Admin"}. Voici un résumé de votre activité.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-semibold text-foreground">Dernières commandes</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            ...
            <div className="p-3 border-t">
              <Link to="/admin/commandes" className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {recentLeads.map((l) => (
              <div key={l.email + l.date} className="flex items-start justify-between p-3 bg-muted/50 rounded-lg">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground">{l.name}</p>
                  <p className="text-xs text-muted-foreground">{l.email} · {l.phone}</p>
                  <p className="text-xs text-muted-foreground/60 mt-0.5">{l.config} · {l.date}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0 ml-2">
                  <a href={`mailto:${l.email}`} className="text-muted-foreground hover:text-foreground"><Mail className="w-4 h-4" /></a>
                  {l.processed && <Badge variant="secondary" className="text-[10px]">Traité</Badge>}
                </div>
              </div>
            ))}
            <Link to="/admin/leads" className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-semibold text-foreground">Alertes & points d'attention</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {unprocessedLeads > 0 && (
            <div className="flex items-start gap-3 p-3 rounded-lg border-l-4 border-orange-400 bg-orange-50">
              <AlertTriangle className="w-4 h-4 text-orange-500 mt-0.5 shrink-0" />
              <p className="text-sm text-foreground/80">{unprocessedLeads} lead{unprocessedLeads > 1 ? "s" : ""} non traité{unprocessedLeads > 1 ? "s" : ""}</p>
            </div>
          )}
          {unprocessedLeads === 0 && (
            <div className="flex items-start gap-3 p-3 rounded-lg border-l-4 border-green-400 bg-green-50">
              <CheckCircle2 className="w-4 h-4 text-green-500 mt-0.5 shrink-0" />
              <p className="text-sm text-foreground/80">Tous les leads sont traités</p>
            </div>
          )}
          <div className="flex items-start gap-3 p-3 rounded-lg border-l-4 border-blue-400 bg-blue-50">
            <Lightbulb className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" />
            <p className="text-sm text-foreground/80">Les données affichées sont en temps réel depuis Supabase.</p>
          </div>
        </CardContent>
      </Card>
    </div>
