  ...
  return (
    <section id="hero" className="relative min-h-[max(70vh,580px)] flex items-center justify-center -mt-20 overflow-hidden">
      ...
      <div className={`relative z-10 max-w-[1400px] mx-auto px-6 lg:px-16 xl:px-24 w-full flex items-center justify-center min-h-[max(70vh,580px)] ${bannerActive ? "pt-[180px]" : "pt-[140px]"}`}>
        <div className="flex flex-col lg:flex-row items-center lg:items-end justify-between w-full gap-12">
          ...
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left max-w-3xl">
            <motion.h1
              ...
              className="font-display text-3xl md:text-4xl lg:text-[2.8rem] xl:text-6xl font-bold leading-[0.95] tracking-tight mb-8 text-white"
            >
          ...
          {/* Right: Stats - moved to right on desktop */}
          {heroStats.length > 0 && (
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.8, delay: 1 }}
              className="flex lg:flex-col items-center lg:items-end gap-4 lg:gap-3"
            >
              {heroStats.map((stat) => (
                <div key={stat.id} className="text-center lg:text-right text-white">
